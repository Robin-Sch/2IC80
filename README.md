...
6. Run `node index.js --mode=alice|bob|mallory --port=x` (replace `x` with the actual serial port of the device, it can be e.g. `/dev/ttyUSB0` on Linux, `COM3` on Windows or `/dev/tty.usbmodem2101` on macOS)